...
{
    GstElement *pipeline, *source, *sink, *enc, *h264p, *rtph264, *bin;
    GstElement *filter0;
    ...
    // g_object_set(G_OBJECT(sink), "host", "192.168.1.7", "port", 5000, NULL);
    ...
    g_object_set(G_OBJECT(enc), "tune", 0x00000004, "bitrate", 500, NULL);
    g_object_set(G_OBJECT(bin), "latency", 0, NULL);

    pipeline = gst_pipeline_new ("test-pipeline");
    gst_bin_add_many (GST_BIN (pipeline), source, filter0, enc, h264p, rtph264, bin, sink, NULL);
